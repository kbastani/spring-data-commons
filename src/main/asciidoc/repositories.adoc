...
For those stores which come with http://www.querydsl.com/[QueryDSL] integration it is possible to derive queries from the attributes contained in a `Request` query string.

Given the `User` object from previous samples the query string
...
The feature will be automatically enabled when Querydsl is found on the classpath.
...
:spring-data-querydsl-sample-source: https://raw.githubusercontent.com/spring-projects/spring-data-examples/master/web/querydsl/src/main/java/example/users/
====
[source,java]
----
include::{spring-data-querydsl-sample-source}web/UserController.java[lines=41;43..51;56..59]
----
====
...
[source,java]
----
include::{spring-data-querydsl-sample-source}UserRepository.java[lines=33..-1]
----
====
